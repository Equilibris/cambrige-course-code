...
public class BumpySphere extends Sphere {
	...
	public Vector3 getNormalAt(Vector3 position) {
		var angle1 = Math.atan2(position.y, position.x);
		var angle2 = Math.atan2(position.x, position.z);

		return new Vector3(Math.cos(angle1), Math.sin(angle1), Math.sin(angle2));
	}
